...
//  console.log("Opening sheet id=%s", ID);
...
function clearNonAssignments() {
  ...
  for (var i = 0, j = 0; i < a.length; i++) {
    //    console.log("%s: %s", i, a[i].emailSent);
    var count = 0;
...
function copyResponses() {
  ...
  for (var i = 1; i < localR.length; i++) {
    ...
    //    console.log("assignments=%s  capacity=%s", assignmentCounts.locals[id], capacityLeft);
...
////////////////////////////////
calculateAffinities = function(locals, internationals, questionImportances) {
    ...
    for (var i = 0; i < locals.length; i++) {
        ...
        local.affinities = {};
        affinities.push([]);
        for (var j = 0; j < internationals.length; j++) {
            var international = internationals[j];
            if (!_.isObject(international.affinities)) {
                international.affinities = {};
            }
            var affinity = calculateAffinity(local, international, questions, questionImportances);
            // // We store the affinity score in each others
            // local.affinities[international.email] = affinity;
            // international.affinities[local.email] = affinity;
            affinities[i].push(affinity);
        };
    };
    return affinities;
}
...
calculateAffinity = function(local, international, questions, importances) {
    var affinity = 0;
    for (var i = 0; i < questions.length; i++) {
        var question = questions[i];
        var importance = importances[question.name].importance || DEFAULT_IMPORTANCE;
        var result = Math.round(importance * question.question(local, international));
        if (result === undefined || isNaN(result)) {
            console.log("Invalid question: " + question.name);
            console.log(importances[question.name]);
            console.log(local);
...
function calcAffinities() {
  ...
  //   }
  // }
  // TODO: dynamically adjust maxAssignments
  maxAssignments = 3;
  ...
  for (var i = 0; i < as.length; i++) {
    ...
    sheet.push(row);
    //    console.log(row.length + "  " + sheet[i].length);
  }
  // affinity sheet (as)
...
function runNetworkFlow() {
  ...
  for (var i = 0; i < affinities.length; i++) {
    ...
    //    console.log("Exchange id: ", row.exchangeId);
  }
  //  console.log(localIds);
  //  console.log("Exchange ids: ", exchangeIds);
  ...
  for (var i = 0; i < flow.length; i++) {
    ...
    if (local < localIds.length && international < exchangeIds.length) {
      //      console.log(local + " " + international);
      //      console.log(localIds[local] + " " + exchangeIds[international]);
...
function fillOutAssignments() {
  ...
  for (var i = 0, col = startCol; i < q.length; i++, col++) {
    ...
    //    console.log("%s: repr %s %s", question.name, typeof question.repr, question.repr);
...
//    console.log(e);
    ...
    for (var j = 0, col = startCol; j < q.length; j++, col++) {
      ...
      if (typeof question.isBad == 'function' && typeof question.repr == 'object') {
        if (question.isBad(l, e)) {
          console.log("isbad %s %s", l.firstName, e.firstName);
    ...
    //    }
    //    console.log("%s: %s", row.length, row);
